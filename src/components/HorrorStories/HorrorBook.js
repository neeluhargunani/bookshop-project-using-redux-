...
const HorrorBook = () => {
     const {products} = useSelector(state => state.ProductsHbReducer);
    return (
        <div className="container">
        <div className="row">
            <div className="col-md-12 col-sm-12 col-xl-12 col-lg-12 text-center py-3 mt-3">
            <h3>Horror Story Books</h3>
        ...
        {products.map(product=>(
         ...
         <Link to={`/hbdetails/${product.id}`}><img src={`/images/${product.image}`} alt="product_image" className="img-thumbnail"/>
      </Link>
        </div>
